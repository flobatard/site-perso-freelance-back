...
const writeUpload = async (dir, name, file) => {
    const buf = Buffer.from(await file.arrayBuffer());
    await writeFile(join(dir, name), buf);
};
const persistSubmission = async (dir, data, logo, photos) => {
...
const escapeHtml = (value) => value
    ...
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
const renderEmail = (id, data, folderPath) => {
    const rows = [
    ...
    const body = rows
        .map(([label, value]) => `<tr><td style="padding:4px 12px 4px 0;vertical-align:top"><strong>${escapeHtml(label)}</strong></td><td style="padding:4px 0;white-space:pre-wrap">${escapeHtml(value || '—')}</td></tr>`)
        .join('');
    return `
...
const sendNotification = async (id, data, folderPath) => {
